# PowerUp V1 Walkthrough V2（历史归档）

> 本文已在 2026-04-08 的“阶段一：软归档”中降级为历史文档。
> 它保留 V1 当时的范围收缩、任务切分与 backlog 取舍记录，
...
- 本文保留了当时为 V1 做范围收缩时的执行说明，目标不是重写 `PRD.md`，而是保留首版 MVP 路线如何被压缩出来的历史上下文。
- `specs-v1/walkthrough.md` 是更早的完整版实施说明；本文是之后的收缩版历史记录，两者都不再承担当前实现真源职责。
- 本文现在只负责保留三类历史信息：
  - 当时认定的阶段性状态快照
  - 当时锁定的 MVP 边界
  - 当时对复杂事项的降级与 backlog 划分

---

## 历史使用说明

- 下文的“当前状态快照”“执行波次”“完成定义”代表当时的规划节点，不代表当前代码现状。
- 如果本文与当前代码、`docs/**` 或 contract 文档冲突，以当前代码和 `docs/**` 为准。
...
| `specs-v1/walkthroughV2.md` | 当时有效的执行顺序、阶段切分与 MVP 发版方案；现已归档为历史记录 |
...
- 如果要回看当时“本轮先做哪一批、后做哪一批”的历史决策，可参考 `walkthroughV2.md`；讨论当前实现时不要以本文为准。
