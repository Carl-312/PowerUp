# PowerUp V1 Walkthrough

## 文档目的

- 本文是 PowerUp V1 的实施说明书，负责把 `specs-v1/PRD.md` 转成可执行任务。
- 本文默认面向“项目 owner + 工程师 + 编程 Agent”协作开发。
- 当前执行以 `specs-v1/walkthroughV2.md` 的收缩口径为准；本文保留原任务 ID、章节骨架与任务卡顺序，方便后续 agent 直接沿用。
- 本文后续出现的“当前 V1 注记 / V2 对齐说明 / 当前不做”，均表示已按 V2 做最小回写；若与原完整版表述冲突，以这些短注记为准。
- 当前 V1 主线只要求浏览、筛选、基础 `/?q=` 搜索、详情页、分类页、关于页和基础 metadata；FTS5、实时 suggestions、公开 API、完整 SEO 硬化、Docker/Caddy 均不作为首版阻塞项。
- 旧文档如存在，只作为参考输入，不参与本轮执行：
  - `../PRD.md`
  - `../walkthrough.md`
